...
public class DialogueTrigger : MonoBehaviour
{
    ...
    {
        DetectPlayer();
        while (_playerInRange && Input.GetKeyDown(triggerKey))
        {
            DialogueManager.Instance.StartDialogue(startDialogueId);
            startDialogueId=DialogueManager.Instance._currentNode.nextId;
            if (startDialogueId == "0"||Input.GetKeyDown(KeyCode.Escape)) break;
        }
    }
